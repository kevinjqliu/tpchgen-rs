...
mod tests {
    ...
    #[test]
    fn positive_bytes_parses_and_validates_values() {
        assert_eq!(parse_positive_bytes("1"), Ok(1));
        assert_eq!(parse_positive_bytes(&i64::MAX.to_string()), Ok(i64::MAX));
        assert_eq!(parse_positive_bytes("8mb"), Ok(8 * 1000 * 1000));
        assert_eq!(parse_positive_bytes("8MiB"), Ok(8 * 1024 * 1024));
        assert_eq!(
            parse_positive_bytes("0"),
            Err("must be greater than zero".to_string())
        );
        assert!(parse_positive_bytes("-1").is_err());
